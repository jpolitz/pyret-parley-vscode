...
# NOTE: function had to be rewritten because pyret-lang does not include t.transform-column(...)
fun transform-column(t, name, f) -> Table:
  fun build-table(wip, columns, column-names) -> Table:
    cases (List) columns:
      | empty => wip
      | link(f-col, r-cols) =>
        cases (List) column-names:
          | link(f-col-name, r-col-names) => 
            build-table(wip.add-column(f-col-name, f-col), r-cols, r-col-names)
        end
    end
  end

  col-names = t.column-names()
  tuple-list = map2(lam(n, col): {n ; col} end, col-names, t.all-columns())
  transformed-columns = tuple-list.map(
    lam(tup):
      if tup.{0} == name: tup.{1}.map(f)
      else: tup.{1}
      end
    end
    )
  base = create-table-with-col(col-names.first, transformed-columns.first)
  build-table(base, transformed-columns.rest, col-names.rest)
end

# create-table-with-col :: (colname :: String, colvals :: List) -> Table
# NOTE: function had to be rewritten because pyret-lang does not include t.rename-column(...)
fun create-table-with-col(colname, colvals):
  base = table: dummy end
  mt-table = base.build-column(colname, lam(r): r["dummy"] end).drop("dummy")
